...
Page {
    ...
    Dialog {
        ...
        onAccepted: {
            ...
            var success = MainSQLConnection.deleteRecord(tablename,
                                                         pkInfo.columnName,
                                                         pkvalue)
            if (success) {
                aRefresh.trigger()
                messageDialog.text = "Запись удалена успешно!"
                messageDialog.open()
            } else {
                messageDialog.text = "При удалении произошла ошибка!"
                messageDialog.open()
            }
        }
    }
    ...
    Action {
        ...
        onTriggered: {
            if (searchColumnCB.displayText !== "" && fieldSearch.text !== "") {
                tableView.tableModel.setFilterQML(searchColumnCB.displayText,
                                                  fieldSearch.text)
            } else {
            ...
            if (sortColumn.displayText !== "") {
                if (!descSort.checked) {
                    tableView.tableModel.setSortQML(sortColumn.displayText,
                                                    TableModel.ASC)
                } else {
                    tableView.tableModel.setSortQML(sortColumn.displayText,
                                                    TableModel.DESC)
    ...
    onTableaccJsonChanged: {
        addButton.visible = tableaccJson.Add
        editButton.visible = tableaccJson.Edit
        deleteButton.visible = tableaccJson.Delete
    }
    ...
    SplitView {
        ...
        EditorElement {
            ...
            columnInfoList: MainSQLConnection.getColumnsInfo(
                                tableView.tablename)
